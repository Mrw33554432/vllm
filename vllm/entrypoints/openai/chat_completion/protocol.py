...
class ChatCompletionResponse(OpenAIBaseModel):
    ...
    # HoleKV: cache ID for this request's input prompt KV trace.
    # The client can use this as holekv_ref in a future request.
    holekv_cache_id: str | None = Field(
        default=None,
        description=(
            "Cache ID for this request's input prompt KV trace. "
            "This can be used as the holekv_ref parameter in a future "
            "request to reuse the cached KV."
        ),
    )
...
class ChatCompletionStreamResponse(OpenAIBaseModel):
    ...
    # HoleKV: cache ID for this request's input prompt KV trace.
    # Only sent on the final chunk of a stream.
    holekv_cache_id: str | None = Field(
        default=None,
        description="Cache ID for this request's input prompt KV trace."
    )
...
class ChatCompletionRequest(OpenAIBaseModel):
    ...
    holekv_ref: str | None = Field(
        default=None,
        description=(
            "Optional reference to a previous HoleKV cache. When provided, "
            "the server tries to reuse the referenced KV cache using the "
            "hole-preserved execution path. The prompt may contain inline "
            "<HOLEKV_REMOVE_START/END> and <HOLEKV_ADD_START/END> markers "
            "to indicate which spans to remove and replace."
        ),
    )
    holekv_owner_id: str | None = Field(
        default=None,
        description=(
            "Optional owner identifier for HoleKV cache access control. "
            "Must match the owner_id stored with the referenced cache."
        ),
    )
    holekv_session_id: str | None = Field(
        default=None,
        description=(
            "Optional session identifier for HoleKV cache access control. "
            "Must match the session_id stored with the referenced cache."
        ),
    )
