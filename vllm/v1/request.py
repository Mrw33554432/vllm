...
class Request:
    def __init__(
        ...
        # HoleKV fields
        self.holekv_ref: str | None = holekv_ref
        self.holekv_owner_id: str | None = holekv_owner_id
        self.holekv_session_id: str | None = holekv_session_id
        self.holekv_cache_id: str | None = None  # Assigned on completion
        self.prompt_text: str | None = prompt_text
        # HoleKV: per-token byte offsets for alignment.
        # Flat list [s0, e0, s1, e1, ...] from EngineCoreRequest.
        self.prompt_token_byte_offsets: list[int] | None = None

        # HoleKV: per-request parsed state, populated by
        # HoleKVEngineProcessor.preprocess_request().
        self.holekv_marker_spans: Any = None
        self.holekv_active_view: Any = None
        self.holekv_position_map: Any = None
    ...
    def from_engine_core_request(
        ...
        return cls(
            ...
            holekv_owner_id=request.holekv_owner_id,
            holekv_session_id=request.holekv_session_id,
            prompt_text=request.prompt_text,
        )
        # Copy HoleKV byte offsets from EngineCoreRequest
        req.prompt_token_byte_offsets = request.prompt_token_byte_offsets
        from vllm.logger import init_logger as _rlog
        if request.prompt_token_byte_offsets:
            _rlog(__name__).info(
                "HoleKV: copied %d byte offsets from EngineCoreRequest to Request",
                len(request.prompt_token_byte_offsets),
            )
        return req
