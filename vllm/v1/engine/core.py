...
class EngineCore:
    ...
    def __init__(
        ...
        # Initialize HoleKV engine processor (disabled by default, enable via env)
        # Import kept local to avoid circular dependencies at module load time.
        from vllm.v1.holekv.engine_processor import HoleKVEngineProcessor

        holekv_enabled = os.environ.get("VLLM_HOLEKV_ENABLED", "0").lower() in (
            "1", "true", "yes", "on"
        )
        self.holekv_processor = HoleKVEngineProcessor(enabled=holekv_enabled)

        # HoleKV: inject the processor into the scheduler so it can create
        # trace records on request completion.
        if holekv_enabled:
            self.scheduler.holekv_processor = self.holekv_processor
            # Wire up the eviction callback: when a trace is evicted from
            # the registry, unpin its KV cache blocks so they can be freed
            # back to the block pool.
            self.holekv_processor.trace_registry.set_block_unpin_fn(
                lambda block_ids: self.scheduler.kv_cache_manager.unpin_blocks(
                    block_ids
                )
            )
            logger.info("HoleKV engine processor enabled")
    ...
    def preprocess_add_request(self, request: EngineCoreRequest) -> tuple[Request, int]:
        ...
        # HoleKV: assign a cache_id to every request.
        # This id will be returned to the client in the response.
        self.holekv_processor.assign_cache_id(req)
        self.holekv_processor.preprocess_request(req)

        # HoleKV ref mode: attempt alignment and active view building.
        # Requires compact token IDs (markers stripped in async_llm.py).
        holekv_mode = getattr(req, '_holekv_mode', None)
        if holekv_mode == 'ref' and not getattr(req, '_holekv_ref_failed', False):
            parsed = getattr(req, '_holekv_parsed', None)
            trace = getattr(req, '_holekv_trace', None)
            if parsed is not None and trace is not None:
                # Use prompt_token_ids which are now compact
                compact_token_ids = list(req.prompt_token_ids) if req.prompt_token_ids else []
                # Convert flat byte offsets from request to tuple pairs for alignment
                flat = req.prompt_token_byte_offsets
                compact_offsets = []
                ...
                alignment = self.holekv_processor.process_alignment(
                    req, compact_token_ids, compact_offsets
                )
                if alignment is not None:
                    # Build active view for scheduler to use
                    d_token_ids = []  # D tokens will be determined during scheduling
                    block_size = getattr(self.scheduler, 'block_size', 16)
                    view = self.holekv_processor.build_active_view(
                        req, parsed, alignment, trace,
                        compact_token_ids, d_token_ids, block_size,
                    )
                    if view is not None and view.is_valid:
                        req.holekv_active_view = view
                        logger.info(
                            "HoleKV active view built for request=%s: "
                            "A=%d M=%d C_old=%d D_hole=%d holes=%d",
                            req.request_id,
                            view.A_length, view.M_length,
                            view.C_old_length, view.D_hole_length,
                            view.hole_length,
                        )
