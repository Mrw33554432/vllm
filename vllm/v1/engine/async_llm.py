...
class AsyncLLM(EngineClient):
    ...
    async def add_request(
        ...
        holekv_ref: str | None = None,
        holekv_owner_id: str | None = None,
        holekv_session_id: str | None = None,
        ...
        if isinstance(prompt, EngineCoreRequest):
            ...
            # HoleKV: strip inline markers BEFORE tokenization
            # so prompt_token_ids are compact (no marker tokens).
            # Keep the original text for preprocess_request to detect markers.
            holekv_original_prompt = None
            holekv_enabled = os.environ.get("VLLM_HOLEKV_ENABLED", "0")
            _hk_check = (
                bool(holekv_enabled)
                and holekv_enabled.lower() in ("1", "true", "yes")
                and isinstance(prompt, dict)
                and "prompt" in prompt
                and isinstance(prompt["prompt"], str)
            )
            if _hk_check:
                try:
                    from vllm.v1.holekv.inline_parser import HoleKVInlineParser

                    holekv_original_prompt = prompt["prompt"]
                    parsed = HoleKVInlineParser().parse(holekv_original_prompt)
                    if parsed.has_markers:
                        compact = parsed.to_compact_prompt()
                        # Store original prompt BEFORE modifying the dict
                        _orig_prompt = holekv_original_prompt
                        # Replace prompt text with compact version for tokenization
                        prompt["prompt"] = compact
                        # CRITICAL: if the prompt dict has a "type" key,
                        # process_inputs treats it as pre-tokenized EngineInput
                        # and IGNORES prompt["prompt"].  Remove "type" so the
                        # input preprocessor re-tokenizes from the compact text.
                        popped_type = None
                        if isinstance(prompt, dict) and "type" in prompt:
                            popped_type = prompt.pop("type")
                            # Also clear pre-tokenized IDs so they are regenerated
                            prompt.pop("prompt_token_ids", None)
                            prompt.pop("prompt_embeds", None)
                        logger.info(
                            "HoleKV: stripped %d markers from prompt "
                            "(%d chars -> %d chars)%s",
                            parsed.num_holes,
                            len(holekv_original_prompt),
                            len(compact),
                            " (re-tokenizing from compact text)"
                            if popped_type else "",
                        )
                except Exception as e:
                    logger.warning(
                        "HoleKV: marker stripping failed: %s", e
                    )
            ...
            request = self.input_processor.process_inputs(
                ...
                holekv_ref=holekv_ref,
                holekv_owner_id=holekv_owner_id,
                holekv_session_id=holekv_session_id,
            )
            # HoleKV: compute per-token byte offsets for alignment.
            # We have the tokenizer here (in the APIServer process).
            # Store on EngineCoreRequest so traces can use them.
            if holekv_original_prompt is not None:
                try:
                    tokenizer = self.renderer.tokenizer
                    if tokenizer and request.prompt_token_ids:
                        # Progressive decode: decode [0:i] tokens incrementally
                        # to get accurate cumulative byte offsets that match
                        # the full decoded text exactly.
                        full_text = tokenizer.decode(request.prompt_token_ids)
                        full_utf8 = full_text.encode("utf-8", errors="replace")
                        offsets = []
                        prev_end = 0
                        for i in range(1, len(request.prompt_token_ids) + 1):
                            partial = tokenizer.decode(
                                request.prompt_token_ids[:i]
                            )
                            partial_bytes = len(
                                partial.encode("utf-8", errors="replace")
                            )
                            offsets.append((prev_end, partial_bytes))
                            prev_end = partial_bytes
                        # Store as flat list for serialization: [s0, e0, s1, e1, ...]
                        flat_offsets = []
                        for s, e in offsets:
                            flat_offsets.extend([s, e])
                        # Store the full decoded text for the trace to use
                        # as rendered_prompt_utf8/rendered_prompt_text
                        import json as _json
                        th = dict(request.trace_headers) if request.trace_headers else {}
                        th["_holekv_byte_offsets"] = _json.dumps(flat_offsets)
                        th["_holekv_full_decoded"] = full_text
                        # Store original prompt (with markers) so the engine core
                        # can parse HoleKV spans.  prompt_text on Request is lost
                        # during IPC; trace_headers reliably survive.
                        th["_holekv_original_prompt"] = holekv_original_prompt
                        request.trace_headers = th
                        # Also set the direct field (may be used if msgspec fix applied)
                        request.prompt_token_byte_offsets = flat_offsets
                        logger.info(
                            "HoleKV: computed %d token byte offsets "
                            "from progressive decode (total %d bytes)",
                            len(offsets), prev_end,
                        )
                except Exception as e:
                    logger.warning("HoleKV: byte offset computation failed: %s", e)

            # Store the ORIGINAL prompt text (with markers) for the engine core's
            # HoleKV preprocess_request to detect and parse inline markers.
            # prompt_token_ids are already compact from tokenization above.
            if holekv_original_prompt is not None:
                request.prompt_text = holekv_original_prompt
            else:
