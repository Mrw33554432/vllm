...
def _extract_boff(request) -> list[int] | None:
    """Extract HoleKV byte offsets from trace_headers Json workaround."""
    import json
    th = getattr(request, 'trace_headers', None) or {}
    raw = th.get("_holekv_byte_offsets")
    if raw:
        return json.loads(raw)
    # Also try the direct field (may be set if msgspec works)
    return getattr(request, 'prompt_token_byte_offsets', None)


def _extract_full_decoded(request) -> str | None:
    """Extract the full decoded prompt text from trace_headers.

    This is the full rendered chat template text that corresponds to
    the byte offsets computed in async_llm.py via progressive decode.
    The trace should use this as rendered_prompt_text/utf8 so that
    byte offsets correctly partition the stored text.
    """
    th = getattr(request, 'trace_headers', None) or {}
    return th.get("_holekv_full_decoded")
...
class Scheduler(SchedulerInterface):
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if request.num_computed_tokens == 0:
                    ...
                    # HoleKV: check for an active HoleKV view.
                    # If a valid view was built during preprocess_request(),
                    # import the C_old reuse-region blocks from the trace
                    # into the request's block table via KVCacheManager.
                    holekv_view = getattr(request, 'holekv_active_view', None)
                    if holekv_view is not None:
                        from vllm.v1.holekv.view_builder import HoleKVActiveView
                        if isinstance(holekv_view, HoleKVActiveView):
                            logger.info(
                                "HoleKV scheduler: found view for request=%s "
                                "is_valid=%s",
                                request_id, holekv_view.is_valid,
                            )
                        if isinstance(holekv_view, HoleKVActiveView) and holekv_view.is_valid:
                            logger.debug(
                                "HoleKV view active for request=%s: "
                                "A=%d M=%d C_old=%d D_hole=%d holes=%d",
                                request_id,
                                holekv_view.A_length,
                                holekv_view.M_length,
                                holekv_view.C_old_length,
                                holekv_view.D_hole_length,
                                holekv_view.hole_length,
                            )
                            # Attempt to import trace blocks for C_old reuse.
                            trace = getattr(request, '_holekv_trace', None)
                            if trace is None:
                                logger.info(
                                    "HoleKV: no _holekv_trace on request=%s "
                                    "(view exists but trace missing)",
                                    request_id,
                                )
                            elif not trace.block_ids_by_group:
                                logger.info(
                                    "HoleKV: trace for %s has empty block_ids_by_group "
                                    "(trace=%s)",
                                    request_id,
                                    request.holekv_ref,
                                )
                            if trace is not None and trace.block_ids_by_group:
                                try:
                                    trace_blk_ids = tuple(trace.block_ids_by_group)
                                    imported_blocks, num_imported = (
                                        self.kv_cache_manager.import_trace_blocks(
                                            request,
                                            trace_blk_ids,
                                            holekv_view.C_old_length,
                                        )
                                    ...
                                    if num_imported > 0:
                                        new_computed_blocks = (
                                            new_computed_blocks + imported_blocks
                                        )
                                        num_new_local_computed_tokens += num_imported
                                        num_computed_tokens += num_imported
                                        logger.info(
                                            "HoleKV imported %d tokens from trace=%s "
                                            "for request=%s",
                                            num_imported,
                                            request.holekv_ref,
                                            request_id,
                                        ...
                                        logger.info(
                                            "HoleKV: trace blocks for %s unavailable "
                                            "(trace=%s, blocks freed), falling back "
                                            "to recomputation",
                                            request_id,
                                            request.holekv_ref,
                                        )
                                    ...
                                    logger.info(
                                        "HoleKV: block import failed for request=%s "
                                        "(trace=%s): %s, falling back to recomputation",
                                        request_id,
                                        request.holekv_ref,
                                        e,
                                    )
            ...
            if (
                ...
                outputs[request.client_index].append(
                    EngineCoreOutput(
                        ...
                        holekv_cache_id=request.holekv_cache_id,
                    )
                )
            else:
    ...
    def _free_request(
        ...
        # HoleKV: create trace record for this completed request.
        # Must be called BEFORE _free_blocks because we need the block IDs.
        holekv_processor = getattr(self, "holekv_processor", None)
        if holekv_processor is not None:
            try:
                flat_block_ids: list[int] = []
                if request.prompt_text:
                    block_ids_raw = self.kv_cache_manager.get_block_ids(
                        request.request_id
                    )
                    # get_block_ids returns tuple[list[int], ...] — one list per
                    # KV cache group. Extract the first group for the trace.
                    if block_ids_raw:
                        first_group = block_ids_raw[0]
                        if first_group:
                            flat_block_ids = list(first_group)
                    full_decoded = _extract_full_decoded(request)
                    holekv_processor.create_trace_record(
                        request_id=request.request_id,
                        ...
                        cache_id=request.holekv_cache_id,
                        prompt_token_byte_offsets=_extract_boff(request),
                        full_decoded_text=full_decoded,
                    )
                # Pin trace blocks so they survive _free_blocks below.
                # Without pinning, blocks are freed immediately and
                # import_trace_blocks() always returns 0.  The registry
                # eviction callback will unpin when the trace is removed.
                if flat_block_ids:
                    self.kv_cache_manager.pin_blocks(flat_block_ids)
            except Exception as e:
                logger.warning(
                    "HoleKV: failed to create trace for request=%s: %s",
