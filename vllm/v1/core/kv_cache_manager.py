...
class KVCacheManager:
    ...
    def pin_blocks(self, block_ids: list[int]) -> None:
        """Pin blocks so they survive free() after the owning request finishes.

        Increments ref_cnt for each block via BlockPool.touch().  After the
        source request is freed the blocks stay alive (ref_cnt > 0) and can
        be reused by a subsequent request via import_trace_blocks().

        Callers MUST arrange for unpin_blocks() to be called when the blocks
        are no longer needed (e.g. on trace eviction).
        """
        if not block_ids:
            return
        blocks_to_touch = []
        for block_id in block_ids:
            if 0 <= block_id < len(self.block_pool.blocks):
                blocks_to_touch.append(self.block_pool.blocks[block_id])
        if blocks_to_touch:
            self.block_pool.touch(blocks_to_touch)

    def unpin_blocks(self, block_ids: list[int]) -> None:
        """Release a previous pin_blocks() call.

        Decrements ref_cnt for each block.  Blocks whose ref_cnt reaches
        zero are returned to the free pool.
        """
        if not block_ids:
            return
        blocks_to_free = []
        ...
        if blocks_to_free:
            self.block_pool.free_block_queue.append_n(blocks_to_free)

    def import_trace_blocks(
        self,
        request: Request,
        trace_block_ids: tuple[list[int], ...],
        num_trace_tokens: int,
    ) -> tuple[KVCacheBlocks, int]:
        """
        Import pre-computed KV blocks from a HoleKV trace for reuse.

        Looks up KVCacheBlock objects by block_id from the block pool,
        returns them as KVCacheBlocks suitable for allocate_slots().
        The coordinator's allocate_new_computed_blocks will handle
        reference counting when these blocks are passed as
        new_computed_blocks.

        Args:
            request: Request to import blocks for.
            trace_block_ids: Block IDs from a HoleKVTraceRecord, one list
                per KV cache group.
            num_trace_tokens: Number of tokens the trace blocks represent.

        Returns:
            Tuple of (KVCacheBlocks for imported blocks, number of tokens
            actually imported). If no blocks could be imported, returns
            (empty, 0).
        """
        if not trace_block_ids or num_trace_tokens <= 0:
            return self.empty_kv_cache_blocks, 0

        imported_groups: list[list[KVCacheBlock]] = []
        tokens_per_block = self.kv_cache_config.kv_cache_groups[0].kv_cache_spec.block_size
        failed = False

        for group_ids in trace_block_ids:
            group_blocks: list[KVCacheBlock] = []
            for block_id in group_ids:
                if block_id < 0 or block_id >= len(self.block_pool.blocks):
                    failed = True
                    break
                block = self.block_pool.blocks[block_id]
                # Only import blocks that are still alive (ref_cnt > 0).
                # Blocks may have been freed when the trace source finished.
                if block.ref_cnt <= 0:
                    failed = True
                    break
            ...
            if failed:
                break
            imported_groups.append(group_blocks)

        if failed or not imported_groups:
            return self.empty_kv_cache_blocks, 0

        num_imported = min(
            num_trace_tokens,
            sum(len(g) for g in imported_groups) * tokens_per_block,
        )
        # Pad to match the KV cache group count. The model expects the
        # same number of groups as kv_cache_config; any groups not filled
        # by the trace get an empty list.
        while len(imported_groups) < self.num_kv_cache_groups:
            imported_groups.append([])
        return self.create_kv_cache_blocks(tuple(imported_groups)), num_imported
